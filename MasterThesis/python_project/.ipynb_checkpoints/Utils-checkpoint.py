...
def process_single_pair(pair, trade_prc, trade_ret, d_open, wait1d, last_day, plot=False):
    ...
    return pair_calcs["payoffs"], pair_calcs["direction"]
...
def calculate_pairs_returns(trade_ret, pairs, d_open=2, wait1d=1):
    ...
    for idx_pair, pair in pairs.iterrows():
        ...
        p_payoffs, p_directions = process_single_pair(
...
def run_portfolio_backtest(dates, port_num, month_id, params, ret, vol, results, portfolios, distance=True):
    ...
    for month_idx in date_range:
        ...
        trades = calculate_pairs_returns( trade_ret, pairs, params['d_open'], params['wait1d'])
...
def main_backtest(ret, vol=None, distance=True):
    ...
    for port_num in range(params['n_trading']):
        ...
        run_portfolio_backtest(dates, port_num, month_id, params, ret, vol, results, portfolios, distance)
